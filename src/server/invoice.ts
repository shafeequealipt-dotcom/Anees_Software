...
export interface InvoiceModel {
  ...
  fileName: string;
  cancelled: boolean;
  seller: { name: string; lines: string[]; taxIdLabel: string; taxId: string | null };
  meta: { k: string; v: string }[];
  party: { heading: string; name: string; lines: string[]; taxId: string | null; taxIdLabel: string };
  columns: { hsn: boolean; discount: boolean; tax: boolean };
  lines: { no: string; desc: string; hsn: string; qty: string; rate: string; discount: string; taxable: string; taxRate: string; tax: string; total: string }[];
...
export async function loadInvoiceModel(db: DB, firmId: number, voucherId: number): Promise<InvoiceModel | null> {
  ...
  const seller = {
    name: firm.name,
    lines: clean([firm.legalName && firm.legalName !== firm.name ? firm.legalName : null, firm.address, [firm.city, firm.pincode].filter(Boolean).join(" "), firm.phone ? `Phone: ${firm.phone}` : null, firm.email, firm.website, R.usesStates && firm.stateCode ? `State: ${firm.stateCode} - ${stateName(firm.stateCode)}` : null]),
  ...
  add(v.type === "sale_invoice" ? "Invoice no." : `${info.label} no.`, number);
  add("Date", formatDate(v.date));
  if (v.dueDate) add(v.type === "quotation" ? "Valid until" : "Due date", formatDate(v.dueDate));
  if (R.usesStates && v.placeOfSupply && info.partySide !== "none") add("Place of supply", `${v.placeOfSupply} - ${stateName(v.placeOfSupply)}`);
  add("Supplier invoice no.", v.supplierInvoiceNo);
  add("PO no.", v.poNumber);
  add("Original invoice", v.originalInvoiceNo);
  add("Vehicle no.", v.vehicleNo);
  add("Transport", v.transportName);
  add("E-way bill", v.ewayBillNo);
  if (v.reverseCharge) add("Reverse charge", "Yes");

  const partyModel = {
    heading: info.partySide === "supplier" ? "Supplier" : v.type === "delivery_challan" ? "Deliver to" : "Bill to",
    name: v.partyName || (info.partySide === "none" ? "" : "Cash"),
    lines: clean([v.billingAddress ?? party?.billingAddress, v.partyPhone ? `Phone: ${v.partyPhone}` : null, R.usesStates && party?.stateCode ? `State: ${party.stateCode} - ${stateName(party.stateCode)}` : null]),
    taxId: v.partyGstin,
    taxIdLabel: R.taxIdLabel,
  };
  ...
  if (info.hasLines) {
    ...
    if (discount > 0 || hasTax) totals.push({ k: "Amount before tax", v: money(v.taxablePaise) });
    if (discount > 0) totals.push({ k: "Discount given", v: money(discount) });
  ...
  if (settings.showBankDetailsOnInvoice && info.outward && info.hasLines) {
    if (firm.bankName) bank.push({ k: "Bank", v: firm.bankName });
    if (firm.bankAccountNo) bank.push({ k: R.country === "SA" ? "IBAN / account" : "Account no.", v: firm.bankAccountNo });
    if (firm.bankIfsc) bank.push({ k: R.country === "SA" ? "SWIFT / code" : "IFSC", v: firm.bankIfsc });
    if (firm.bankBranch) bank.push({ k: "Branch", v: firm.bankBranch });
  ...
  return {
    ...
    split,
    totals,
    words: amountInWords(v.totalPaise),
    bank,
    qr: zatca ? { src: zatca, caption: "ZATCA QR code" } : upi ? { src: upi, caption: "Scan to pay (UPI)" } : null,
