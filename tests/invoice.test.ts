...
describe("invoice PDF", () => {
  it("Saudi tax invoice: VAT in one column, ZATCA QR, amount in riyals", async () => {
    ...
    await db.update(firms).set({ bankName: "Al Rajhi", bankAccountNo: "SA0380000000608010167519", address: "Riyadh" }).where(eq(firms.id, f.id));
    const vat = (await db.select().from(taxRates).where(eq(taxRates.firmId, f.id))).find((r) => r.gstBp === 1500)!;
    const cust = await saveParty(db, f.id, { name: "Riyadh Stores", gstin: "310000000000003", kind: "customer", billingAddress: "Olaya St" }, 1);
    const item = await saveItem(db, f.id, { name: "Dates 1kg", salePricePaise: 5000, taxRateId: vat.id, openingQtyMilli: 100000 }, 1);
    ...
    expect(m.qr?.caption).toBe("ZATCA QR code");
    expect(m.totals.map((t) => t.k)).toEqual(expect.arrayContaining(["Amount before tax", "VAT", "Total"]));
    expect(m.totals.find((t) => t.k === "Total")!.v).toBe("575.00");
    expect(m.words).toMatch(/Riyals/);
    expect(m.bank.map((b) => b.k)).toContain("IBAN / account");
