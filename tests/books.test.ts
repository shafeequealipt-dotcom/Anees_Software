...
beforeAll(async () => {
  ...
  bankId = await saveAccount(db, accountSchema.parse({ kind: "bank", name: "HDFC Current", ifsc: "HDFC0001234", openingBalancePaise: 5_000_000 }) as never, 1);
  ...
  customer = await saveParty(db, { kind: "customer", name: "Ravi Stores", phone: "9876543210", stateCode: "27", openingBalancePaise: 100_000, creditDays: 15 }, 1);
  outOfState = await saveParty(db, { kind: "customer", name: "Bengaluru Mart", stateCode: "29" }, 1);
  supplier = await saveParty(db, { kind: "supplier", name: "Paper Mills", stateCode: "27", openingBalancePaise: -50_000 }, 1);

  pen = await saveItem(db, { name: "Blue pen", code: "PEN01", hsn: "9608", unitId: pcs, altUnitId: box, altUnitFactorMilli: 10_000, salePricePaise: 1_000, purchasePricePaise: 600, taxRateId: gst18, openingQtyMilli: 100_000, openingRatePaise: 600, minStockMilli: 20_000 }, 1);
  notebook = await saveItem(db, { name: "Notebook A5", hsn: "4820", unitId: pcs, salePricePaise: 5_000, purchasePricePaise: 3_000, taxRateId: gst5 }, 1);
  service = await saveItem(db, { kind: "service", name: "Binding service", hsn: "998912", salePricePaise: 2_000, taxRateId: gst18 }, 1);
});

describe("books end to end", () => {
  it("rejects duplicate names and bad GSTINs", async () => {
    await expect(saveParty(db, { name: "ravi stores" }, 1)).rejects.toThrow(/already exists/);
    await expect(saveParty(db, { name: "Someone", gstin: "27ABCDE1234F1ZZ" }, 1)).rejects.toThrow();
    await expect(saveItem(db, { name: "Other pen", code: "PEN01" }, 1)).rejects.toThrow(/already used/);
  });
  ...
  it("sells on credit with part payment, within the state", async () => {
    ...
    const v = (await getVoucher(db, res.id))!;
  ...
  it("charges IGST to another state and uses a walk-in cash bill", async () => {
    ...
    const c = (await getVoucher(db, cash.id))!;
  ...
  it("settles oldest bills first when a payment comes in", async () => {
    const p = await saveVoucher(db, { type: "payment_in", date: "2026-09-05", partyId: customer, amountPaise: 20_000, accountId: bankId, paymentMode: "UPI" }, 1);
    const pay = (await getVoucher(db, p.id))!;
  ...
  it("buys stock on credit and pays part of it", async () => {
    ...
    expect((await getVoucher(db, r.id))!.voucher.totalPaise).toBe(157_500);
    await saveVoucher(db, { type: "payment_out", date: "2026-09-10", partyId: supplier, amountPaise: 100_000, accountId: bankId, paymentMode: "Bank transfer" }, 1);
    const sup = (await partyBalances(db)).find((p) => p.id === supplier)!;
  ...
  it("records a sale return against the invoice", async () => {
    const [inv] = await listVouchers(db, { types: ["sale_invoice"], partyId: customer });
    const r = await saveVoucher(db, {
      type: "credit_note",
      date: "2026-09-06",
      partyId: customer,
  ...
  it("edits an invoice and rewrites balances, stock and numbers", async () => {
    const [inv] = (await listVouchers(db, { types: ["sale_invoice"], partyId: outOfState }));
    const before = (await getVoucher(db, inv.id))!;
    await saveVoucher(db, {
      id: inv.id,
      type: "sale_invoice",
    ...
    const after = (await getVoucher(db, inv.id))!;
    ...
    await expect(saveVoucher(db, { type: "sale_invoice", number: 1, date: "2026-09-09", partyName: "x", lines: [{ description: "x", qtyMilli: 1000, ratePaise: 100 }] }, 1)).rejects.toThrow(/already used/);
  });

  it("cancelling removes the effect but keeps the record", async () => {
    const r = await saveVoucher(db, { type: "expense", date: "2026-09-07", roundOff: false, accountId: cashId, lines: [{ description: "Shop rent", qtyMilli: 1_000, ratePaise: 200_000 }] }, 1);
    const cashBefore = (await accountStatement(db, cashId, "2026-01-01", "2026-12-31")).closingPaise;
    await cancelVoucher(db, r.id, 1);
    ...
    expect((await getVoucher(db, r.id))!.voucher.status).toBe("cancelled");
  });

  it("moves money between accounts and adjusts stock", async () => {
    await saveVoucher(db, { type: "money_transfer", date: "2026-09-08", amountPaise: 5_000, accountId: cashId, toAccountId: bankId }, 1);
    await expect(saveVoucher(db, { type: "money_transfer", date: "2026-09-08", amountPaise: 5_000, accountId: cashId, toAccountId: cashId }, 1)).rejects.toThrow(VoucherError);
    const adj = await saveVoucher(db, { type: "stock_adjustment", date: "2026-09-08", direction: -1, lines: [{ itemId: pen, description: "Blue pen", qtyMilli: 70_000, ratePaise: 0 }] }, 1);
    expect(adj.warnings).toHaveLength(0);
    const low = await stockSummary(db, { lowOnly: true });
    expect(low.map((s) => s.id)).toContain(pen);
  });

  it("produces consistent reports", async () => {
    const pl = await profitAndLoss(db, "2026-09-01", "2026-09-30");
    ...
    const book = await dayBook(db, "2026-09-01", "2026-09-30");
    expect(book.length).toBeGreaterThan(5);
    const bySale = await itemSales(db, "2026-09-01", "2026-09-30");
  ...
  it("keeps items with history instead of deleting them", async () => {
    ...
    expect(await deleteItem(db, pen, 1)).toBe("deactivated");
